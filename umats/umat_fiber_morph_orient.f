...
c...  calculate left stretch tensor v from F
     ...
     #     + dfgrd1(2,3)*dfgrd1(3,3)
      ...
      print *, V(1,1), V(2,2), V(3,3)

c...  calculate deformed fiber in local rotated coordinate system
      norm = sqrt(xx(1)*xx(1) + xx(2)*xx(2) + xx(3)*xx(3))
      xf(1) = (V(1,1)*xx(1) + V(1,2)*xx(2) + V(1,3)*xx(3))/norm
      xf(2) = (V(2,1)*xx(1) + V(2,2)*xx(2) + V(2,3)*xx(3))/norm
      xf(3) = (V(3,1)*xx(1) + V(3,2)*xx(2) + V(3,3)*xx(3))/norm
...
c...  calculate reference fiber in local rotated coordinate system
      xf0(1) = finv(1,1)*xf(1) + finv(1,2)*xf(2) + finv(1,3)*xf(3)
      xf0(2) = finv(2,1)*xf(1) + finv(2,2)*xf(2) + finv(2,3)*xf(3)
      xf0(3) = finv(3,1)*xf(1) + finv(3,2)*xf(2) + finv(3,3)*xf(3)

c...  calculate update growth factor
